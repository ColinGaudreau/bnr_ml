...
class FastRCNNDetector(object):
	...
	def __init__(
		...
		def reshape_loc_layer(loc_layer, num_classes):
			return loc_layer.reshape((-1, num_classes, 4))

		self.detect = get_output(network['detect'], deterministic=False)
		self.detect_test = get_output(network['detect'], deterministic=True)
		self.localize = reshape_loc_layer(get_output(network['localize'], deterministic=False))
		self.localize_test = reshape_loc_layer(get_output(network['localize'], deterministic=True))
	...
	def _get_cost(self, detection_output, localization_output, target, lmbda=1.):
		...
		class_idx = target[:,-self.num_classes:].argmax(axis=1)
		mask = T.ones((target.shape[0], 1))
		mask = T.switch(T.eq(target[:,-self.num_classes:].sum(axis=1,keepdims=True), 0), 0, 1) # mask for non-object ground truth labels

		cost = categorical_crossentropy(detection_output, target[:,:self.num_classes])
	...
	'''
		...
		print_obj.print('Compiling took %.3f seconds' % (time.time() - ti,))

		train_loss, test_loss = np.zeros(epochs), np.zeros(epochs)

		print_obj('Beginning training')
	...
	def generate_data(
		...
		for i in range(0,annotations.__len__(),2):
			X, y = np.zeros((num_rios * per_batch, 3) + new_size), np.zeros((num_rios * per_batch, 4 + num_classes))
			cnt = 0
			for j in range(per_batch):
				...
				for k in range(num_rios):
					coord, label = np.zeros(4), np.zeros(num_classes)
					...
					y[cnt,:4], y[cnt,-num_classes:] = coord, label
					cnt += 1
			yield X[:cnt], y[:cnt]
