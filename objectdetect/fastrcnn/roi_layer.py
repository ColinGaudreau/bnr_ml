...
roi_mod = SourceModule("""
    ...
    __global__ void roi_pool_grad(tens4 *x_pool_grad, tens4 *x, tens3 *boxes, int rows, int cols)
    {
        // declare data here
        ...
        for(m=0; m < boxes->shp.dim2; m++)
        {
            ...
            for(i=0; i<rows; i++)
            {
                ...
                {
                    // define column/rows to loop over
                    r_min = floorf(i * (yf - yi) / rows);
                    c_min = floorf(j * (xf - xi) / cols);
                    r_max = min(ceilf((i + 1) * (yf - yi) / rows), yf);
                    c_max = min(ceilf((j + 1) * (xf - xi) / cols), xf);
...
class PyCUDAROIPool(theano.Op):
    ...
    def grad(self, inputs, grads):
        ...
        x_grad = PyCUDAROIPoolGrad(self.shape)(x, boxes)
...
class PyCUDAROIPoolGrad(theano.Op):
    ...
    def make_thunk(self, node, storage_map, _, _2, impl=None):
        ...
        def thunk():
            ...
            block = (1,1,1)
            pycuda_func(z_ptr, x_ptr, boxes_ptr, np.int32(self.shape[0]), np.int32(self.shape[1]), block=block, grid=grid)
        return thunk
...
class ROILayer(Layer):
    ...
    def get_output_shape_for(self, input_shape):
        return (input_shape[0] * self.boxes.shape[1], self.shape[0], self.shape[1])
